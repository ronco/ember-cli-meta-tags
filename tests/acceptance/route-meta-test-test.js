...
module('Acceptance: RouteMetaMixin', function(hooks) {
  ...
  test('Can set dir on application route', async function(assert) {
    ...
    assert.equal(
      await document.querySelector('meta[property="og:title"]').getAttribute('dir'),
      'ltr'
    );
  });
  ...
  test('Sets and clears meta', async function(assert) {
    ...
    assert.equal(
      await document.querySelector('meta[property="og:name"]').getAttribute('content'),
      'Ice-T'
    );
    await visit('/route-2');
    assert.equal(
  ...
  test('has meta for all route-meta routes in hierarchy', async function(assert) {
    ...
    assert.equal(
      await document.querySelector('meta[property="og:type"]').getAttribute('content'),
      'Root'
    );
  });
